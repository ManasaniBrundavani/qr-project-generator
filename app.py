...
# Ensure this matches the folder for static serving
VIDEO_UPLOAD_DIR = "static"
ALLOWED_VIDEO_EXTS = {".mp4", ".webm", ".ogg", ".m4v", ".mov"}
...
if project_id:
    ...
    if project:
        st.title(project[3])  # Project Title
        st.write(f"**Student Name:** {project[1]}")
        st.write(f"**Roll Number:** {project[2]}")
        st.write(f"**Description:** {project[4]}")

        video_link = project[6]

        if video_link:
            # Check if it is a local path stored in the 'static' folder
            if video_link.startswith("static/") or "static" in video_link:
                # Direct relative path works because of enableStaticServing
                st.video(video_link)
            elif "uploaded_videos" in video_link:
                # Backward compatibility for old folder name
                fixed_path = video_link.replace("uploaded_videos", "static")
                st.video(fixed_path)
            else:
                # It's a YouTube or external web link
                st.video(video_link)
    else:
        st.error("Project not found")
    st.stop()
...
def compute_qr_base_url(settings):
    # Hardcoded your Streamlit Cloud URL for consistency
    return "https://qr-project-generator-mtuimjvrgnb5vs9ojfgazq.streamlit.app"
...
def save_uploaded_video(uploaded_file, qr_base_url):
    ...
    # Save the file to the local 'static' folder
    with open(file_path, "wb") as f:
        f.write(uploaded_file.getbuffer())

    # Return the path starting with static/ for the DB
    return f"static/{file_name}"

def create_project_and_qr(name, roll, project_title, project_description, video_link, qr_base_url, settings):
    existing_id = find_existing_project_id(roll, project_title)
    if existing_id:
        project_id = existing_id
        update_project(project_id, name, roll, project_title, project_description, video_link, video_link)
    else:
        project_id = insert_project(name, roll, project_title, project_description, video_link, video_link, "")
    
    unique_url = f"{qr_base_url}/?id={project_id}"
    file_name = safe_filename(f"{roll}_{name}_{project_title}_{project_id}")
    qr_path = generate_qr(unique_url, file_name, label=f"{name} | {roll}")
    set_qr_path(project_id, qr_path)
    return project_id, unique_url, qr_path
...
st.set_page_config(page_title="NEX AI QR Registration", page_icon="🎓", layout="wide")

st.title("🎓 NEX AI Project Registration System")

menu = st.sidebar.selectbox("Menu", ["Register Project", "View All Projects", "Admin Settings"])
...
if menu == "Register Project":
    ...
    with st.form("registration_form"):
        ...
        roll = st.text_input("Roll Number")
        project_title = st.text_input("Project Title")
        video_web_link = st.text_input("Project Website/YouTube Link (Optional)")
        local_video_file = st.file_uploader("Or Upload Video File", type=list(ALLOWED_VIDEO_EXTS))
        project_description = st.text_area("Project Description")
        submit = st.form_submit_button("Generate QR & Register")

    if submit:
        if all([name, roll, project_title, project_description]):
            # If a local file is uploaded, save it and get the static/ path
            local_video_path = save_uploaded_video(local_video_file, QR_BASE_URL)
            
            # Use web link if provided, otherwise use the uploaded file path
            final_video_link = video_web_link if video_web_link else local_video_path

            _, unique_url, qr_path = create_project_and_qr(
                clean_text(name), clean_text(roll), clean_text(project_title),
                clean_text(project_description), final_video_link, QR_BASE_URL, settings
            )
            st.success("Registration successful!")
            st.image(qr_path, width=250, caption="Scan this QR Code")
            st.code(unique_url)
        else:
            st.error("Please fill all required fields.")
    ...
    if data:
        ...
        for idx, row in enumerate(data):
            with cols[idx % 4]:
                ...
                if is_valid_image_path(row[7]):
                    st.image(row[7])
    else:
        st.info("No projects found.")

elif menu == "Admin Settings":
    st.write(f"Current Base URL: {QR_BASE_URL}")
